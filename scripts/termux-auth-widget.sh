...
# OpenSoul Auth Widget for Termux
...
# Check auth status
termux-toast "Checking OpenSoul auth..."

STATUS=$(ssh "$SERVER" '$HOME/opensoul/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/opensoul/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|OPENSOUL_EXPIRING|OPENSOUL_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/opensoul/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|OPENSOUL_EXPIRED|OPENSOUL_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: OpenSoul won't work until re-auth"
                ;;
        esac
        ;;
