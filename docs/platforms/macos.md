---
summary: "OpenSoul macOS companion app (menu bar + gateway broker)"
read_when:
...
# OpenSoul macOS Companion (menu bar + gateway broker)

The macOS app is the **menu‑bar companion** for OpenSoul. It owns permissions,
...
- Installs the global CLI (`opensoul`) via npm/pnpm on request (bun not recommended for the Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `opensoul gateway install`.
...
The app registers the `opensoul://` URL scheme for local actions.
...
1. Install and launch **OpenSoul.app**.
...
Tip: compare against `opensoul gateway discover --json` to see whether the
