---
summary: "Updating OpenSoul safely (global install or source), plus rollback strategy"
read_when:
  - Updating OpenSoul
...
detects existing installs, upgrades in place, and runs `opensoul doctor` when
needed.
...
- For **global installs**, the script uses `npm install -g opensoul@latest` under the hood.
- Legacy note: `opensoul` remains available as a compatibility shim.
...
- If your Gateway runs as a service, `opensoul gateway restart` is preferred over killing PIDs.
...
- Installs deps, builds, builds the Control UI, and runs `opensoul doctor`.
...
1. Runs the same source-update flow as `opensoul update` (git checkout only).
...
Tip: to see the current published version, run `npm view opensoul version`.
...
- Run `opensoul doctor` again and read the output carefully (it often tells you the fix).
