...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in OpenSoul. See [Webhook Hooks](/automation/webhook) or use `opensoul webhooks` for Gmail helper commands.
...
Hook packs are standard npm packages that export one or more hooks via `opensoul.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.opensoul/hooks/<id>`.
