...
OpenSoul 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`opensoul gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均支持）：

- 向导（推荐）：`opensoul onboard --install-daemon`
- 直接安装：`opensoul gateway install`
- 配置流程：`opensoul configure` → 选择 **Gateway service**
- 修复/迁移：`opensoul doctor`（提供安装或修复服务）

服务目标取决于操作系统：

- macOS：LaunchAgent（`ai.opensoul.gateway` 或 `ai.opensoul.<profile>`；旧版 `com.opensoul.*`）
- Linux/WSL2：systemd 用户服务（`opensoul-gateway[-<profile>].service`）
