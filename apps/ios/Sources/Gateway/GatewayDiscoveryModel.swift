...
final class GatewayDiscoveryModel {
    ...
    func start() {
        ...
        for domain in OpenSoulBonjour.gatewayServiceDomains {
            let params = NWParameters.tcp
            params.includePeerToPeer = true
            let browser = NWBrowser(
                for: .bonjour(type: OpenSoulBonjour.gatewayServiceType, domain: domain),
                using: params)
            ...
            browser.start(queue: DispatchQueue(label: "ai.opensoul.ios.gateway-discovery.\(domain)"))
        }
    }
    ...
    private static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (OpenSoul)", with: "")
