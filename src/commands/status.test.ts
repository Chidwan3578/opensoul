...
beforeAll(() => {
  previousProfile = process.env.OPENSOUL_PROFILE;
  process.env.OPENSOUL_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.OPENSOUL_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/opensoul-root.js", () => ({
  resolveOpenSoulPackageRoot: vi.fn().mockResolvedValue("/tmp/opensoul"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/opensoul/pnpm-lock.yaml",
      markerPath: "/tmp/opensoul/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.opensoul.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.opensoul.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("OpenSoul status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("opensoul status --all") ||
          l.includes("opensoul --profile isolated status --all") ||
          l.includes("opensoul status --all") ||
          l.includes("opensoul --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    const prevToken = process.env.OPENSOUL_GATEWAY_TOKEN;
    process.env.OPENSOUL_GATEWAY_TOKEN = "abcd1234";
    try {
      ...
      if (prevToken === undefined) {
        delete process.env.OPENSOUL_GATEWAY_TOKEN;
      } else {
        process.env.OPENSOUL_GATEWAY_TOKEN = prevToken;
      }
    }
  });
